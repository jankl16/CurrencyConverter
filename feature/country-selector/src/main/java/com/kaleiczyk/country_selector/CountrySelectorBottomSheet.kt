...
private fun CountrySelectorBottomSheet(
    ...
    Column(
        ...
        val title = when (type) {
            CountrySelectorType.SELECT_SENDER -> R.string.country_selector_receiving_from_title
            CountrySelectorType.SELECT_RECEIVER -> R.string.country_selector_sending_to_title
        }
        Text(
...
private fun SearchTextField(query: String, onQueryChanged: (String) -> Unit) {
    OutlinedTextField(
        ...
        label = {
            Text(
                stringResource(id = R.string.country_selector_text_field_label_title),
